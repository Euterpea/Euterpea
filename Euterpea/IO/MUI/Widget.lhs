...
>   returnA -< y
>   --loop $ second (initUI i) >>> arr (uncurry inj) >>> mkUISF aux
>     where
...
> toggle initUI layout draw = 
>   mkUI initUI layout draw (const nullSound) pair process id
>   where
...
>   rec lst <- initUI (take n $ repeat (h-buffer)) -< take n $ (adjust i:lst)
...
>   rec lst <- initUI (take n $ repeat (h-buffer)) -< take n $ (reverse (map adjust is) ++ lst)
>   canvas (w,h) -< Just $ withColor color $ polyline $ zip xs lst
>   where xs = reverse $ map truncate [0,(fromIntegral w / fromIntegral (n-1))..(fromIntegral w)]
>         adjust i = buffer + truncate (fromIntegral (h - 2*buffer) * (1 - i))
...
> delayt :: UISF (Time, Double, Event b) (Event b)
> delayt = proc (t, d, e) -> do
>   rec q <- initUI [] -< maybe q' (\e' -> q' ++ [(t,e')]) e
>       let (ret, q') = case q of
>               [] -> (Nothing, q)
